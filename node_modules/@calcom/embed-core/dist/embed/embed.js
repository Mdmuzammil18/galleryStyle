!function(){const M=({buttonText:a,buttonClasses:t,buttonColor:e,buttonTextColor:o})=>`<button class="z-[999999999999] hidden fixed md:bottom-6 bottom-4 md:right-10 right-4 md:left-10 left-4 ${t.join(" ")} flex h-16 origin-center bg-red-50 transform cursor-pointer items-center
...
<slot></slot>`;class H extends HTMLElement{static get observedAttributes(){return["loading"]}assertHasShadowRoot(){if(!this.shadowRoot)throw new Error("No shadow root")}attributeChangedCallback(t,e,o){this.assertHasShadowRoot();const r=this.shadowRoot.querySelector(".loader"),i=this.shadowRoot.querySelector("#error"),n=this.shadowRoot.querySelector("slot");if(!r||!n||!i)throw new Error("One of loaderEl, slotEl or errorEl is missing");if(t==="loading"){if(o=="done")r.style.display="none";else if(o==="failed"){r.style.display="none",n.style.visibility="hidden",i.style.display="block";const s=E(this.dataset.errorCode);i.innerText=s}}}constructor(){super(),this.attachShadow({mode:"open"}),this.assertHasShadowRoot(),this.shadowRoot.innerHTML=`<style>${window.Cal.__css}</style><style>${$}</style>${N}`}}const T=`@keyframes loader{0%{transform:rotate(0)}25%{transform:rotate(180deg)}50%{transform:rotate(180deg)}75%{transform:rotate(360deg)}to{transform:rotate(360deg)}}@keyframes loader-inner{0%{height:0%}25%{height:0%}50%{height:100%}75%{height:100%}to{height:0%}}.loader-inner{vertical-align:top;display:inline-block;width:100%;animation:loader-inner 2s infinite ease-in}.loader{display:block;width:30px;height:30px;position:relative;border-width:4px;border-style:solid;animation:loader 2s infinite ease}.loader.modal-loader{margin:60px auto}
`,j=`<style>
.my-backdrop {
  position:fixed;
  width:100%;
